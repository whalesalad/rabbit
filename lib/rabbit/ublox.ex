defmodule Rabbit.Ublox do
  ...
  def encode(packet) do
    msg_class = classes(packet.msg_class)
    msg_id = apply(Ublox, packet.msg_class, [packet.msg_id])
    payload_length = Kernel.length(packet.payload)

    prefix = <<0xB5, 0x62>>

    # The body is the portion of the message that is checksummed.
    body = <<msg_class, msg_id, payload_length::16-little>>
